...
impl NetConfig {
    ...
    pub fn init<B: Backend>(&self, device: &B::Device) -> Net<B> {
        ...
        for _i in 0..self.num_res_blocks {
        ...
        Net {
            ...
            value_act2: Relu::new(),
            value_fc2: LinearConfig::new(256, 3).init(device),
        }
    }
}
impl<B: Backend> Net<B> {
    pub fn forward(&self, x: Tensor<B,4>) ->(Tensor<B, 2>, Tensor<B, 2>) {
        ...
        let policy = policy.reshape([-1, (16 * BOARD_FILES * BOARD_RANKS) as i32]);
        ...
        let value = value.reshape([-1, (8 * BOARD_FILES * BOARD_RANKS) as i32]);
        let value = self.value_fc1.forward(value);
        let value = self.value_act2.forward(value);
